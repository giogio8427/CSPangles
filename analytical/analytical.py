...
while (newSim=='y'):
    ...
    if geom=="sphere":
      ...
      X,Y,Z=pltSphere.prismCoord(ro)
      X1,Y1,Z1=pltSphere.rectCoord(rr)
      tt=pltSphere.plotRect(X1,Y1,Z1, clrMatrix=thetaRTime[:,:,timeSel]*thetaIn+Tinf, 
                                ccLim=(ccMin, ccMax), colorMap=cMap)
      tt2=pltSphere.plotPrism(X,Y,Z, clrMatrix=np.ones(X.shape)*thetaRo[timeSel]*thetaIn+Tinf, ccLim=(ccMin, ccMax),colorMap=cMap)
      tt3=pltSphere.plotPrism(X,Y*0.0,Z, clrMatrix=np.ones(X.shape)*thetaLump[timeSel]*thetaIn+Tinf, 
                                ccLim=(ccMinLump, ccMaxLump),colorMap=cMap)
      tt4=pltSphere.plotPrism(X,Y,Z, clrMatrix=np.ones(X.shape)*thetaLump[timeSel]*thetaIn+Tinf, ccLim=(ccMinLump, ccMaxLump),colorMap=cMap)   
