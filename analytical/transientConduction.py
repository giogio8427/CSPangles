# Description: This script calculates the transient conduction heat transfer in a sphere, cylinder or slab.\n   
# The script uses the analytical solution for 1D transient heat conduction for a solid sphere, cylinder, or slab shape.\n   
# The solution is based on the zeta, Bi equation and the theta function.\n  
#References: 
#1) Recktenwald 2006
...
# Initialize arrays
    ...
    thetaRo, temp = theta(rs, b, rootsVal, Fo)   # dimensionless temperature profile
    thetaRo[0]=1.
    ...
    thetaR, temp = theta(rc, b, rootsVal, Fo)    # dimensionless temperature profile
    thetaR[0]=1.
    T_r = Tinf + thetaR*(Ti-Tinf)       # convert theta to temperature in Kelvin, K

    thetaMid, temp =theta(0.5, b, rootsVal, Fo)    # dimensionless temperature profile
    thetaMid[0]=1.
    ...
    for ii in range(0,len(rr),1):
        thetaRadius[ii,:],dThetaMin[ii,:] = theta(rr[ii]/ro, b, rootsVal, Fo) 
    ...
    energy_enMax_ratio=abs(energy/energyMax)
    ...
    for i in range(0,len(t),nSliderSteps):
        table_data.append([t[i], Fo[i], thetaR[i], thetaRo[i], T_r[i], T_o[i], energy[i], energy_enMax_ratio[i], max(dThetaMin[:,i]), (Bi**2)*Fo[i]])
    ...
    +-------------------------------------------------------------+
        """)
    headers = ["time [s]", "Fou [-]", "theta* center [-]", "theta* outer [-]", "Tcenter [°C]", "Touter [°C]", "Energy [J]", "En./En.max [-]", "dThetaMax [-]", "(Bi**2)*Fou [-]"]
    print(tabulate(table_data, headers=headers, floatfmt=(".4f", ".4f", ".4f", ".4f", ".4f", ".4f", ".4f", ".4f", ".3e", ".4f")))
    ...
    for j in arraySliderSteps:
        slider_steps.append({
            ...
            "args": [{
                ...
                "title": f"{intStr} - Time Step: {t[j]} - Energy {energyStr}: {energy[j]:.2f}  E/E_Max: {energy_enMax_ratio[j]:.3f} E/E_Lumped: {energyRatio[j]:.3f}   Biot: {Bi:.2f} Fourier: {Fo[j]:.4f}",
                "shapes": [{
                    **cursor_line,
                    "x0": t[j],
                    "x1": t[j],
