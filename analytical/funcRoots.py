...
from numpy import sign, diff, zeros, pi, where, sign
...
def roots(z, b, Bi):
    ...
    diffVal = diff(signVal)
    
    # b as the shape factor where 2 sphere, 1 cylinder, 0 slab
    # use the where function to find array index of value change
    # note that where returns a tuple
    if b == 2 or b==0:
        # location of sphere or slab roots
        # find index of only positive value change thus ignoring singularities
        whereVal = where(diffVal>0)
    elif b==1:
        # location of cylinder roots
        whereVal = where(diffVal)
    
    roots = zeros(len(whereVal[0])) # setup an empty array to store roots
    
    for i, j in enumerate(whereVal[0]):
       roots[i] = brentq(func, z[j], z[j+1], args=(Bi)) # roots for a given interval
...
#         where = where(diff)
#     ii=range(0,800,1)
#     roots = zeros(len(ii)) # setup an empty array to store roots
#     n=0
#     for i in ii:
#         n=n+1
#         if Bi==1.0:
